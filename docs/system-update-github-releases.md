...
The combined RC12 release is an explicit exception: tag
`hardened-system-0.3.0-raw.12` carries both app `2.0.42` metadata and raw/SD
...
system URL validation. Existing installations manually install the app `2.0.42`
key-transition prerelease first; only then should they use
`hardened-system-preview`. The stable system channel is not moved by this RC.
...
The current preview candidate is combined RC12, app `2.0.42` plus system
`0.3.0-raw.12`, on tag `hardened-system-0.3.0-raw.12`. It uses Buildroot
`2026.05.1` and Linux `5.10.265-tag-`; metadata format 2 requires app `2.0.42`
and is signed by key ID `hardened-system-prod-2026q3`. The stable channel
remains on `0.2.23-raw.1` until a separate promotion decision.
...
The current stable metadata intentionally separates several version fields:
...
The stable channel is still the retained lab baseline. It is intended to validate the sysupgrade
...
- `format`: currently `hardened-nanokvm-system-update-v2` for raw.12.
...
- `required_app_version`: minimum application version, currently `2.0.42`.

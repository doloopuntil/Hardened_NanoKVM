...
The current `hardened-system-stable` channel points to the RC6 raw payload
`0.2.20-raw.1`, published on companion tag
`hardened-system-0.2.20-raw.1`. The full `hardened-rust-rc6` release also
...
tags. The raw payload was built from the RC6 `2.0.28` Hardened SD image and
contains gzip-compressed boot/rootfs partition images plus matching channel
metadata. RC6 keeps the tested raw-update baseline and includes password reset
removal, Rust-side watchdog/network state migration, mass-storage image
validation, startup NTP sync, WebRTC keyframe recovery, and opt-in USB HID
wake-on-write from the application payload.

The current metadata intentionally separates several version fields:

- system update version: `0.2.20-raw.1`;

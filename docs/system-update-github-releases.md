...
The current `hardened-system-stable` channel points to the RC5 raw payload
`0.2.19-raw.1`, published on companion tag
`hardened-system-0.2.19-raw.1`. The full `hardened-rust-rc5` release also
...
tags. The raw payload was built from the RC5 `2.0.27` Hardened SD image and
contains gzip-compressed boot/rootfs partition images plus matching channel
metadata. RC5 keeps the tested raw-update baseline and includes the repository
cleanup, native runtime relocation, package validation, and System settings UI
refinements from the application payload.

The current metadata intentionally separates several version fields:

- system update version: `0.2.19-raw.1`;
